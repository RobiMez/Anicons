        ...
        def check_delta(lfd):
            ...
            for ep in data :
                ...
                if lfd['episode_names'] != {}:
                    try:
                        prediction_data = lfd['episode_names'][int(prediction[0])]
                    ...
                    print(f'{c.red}Could not find Episode data from the internet {c.o}\n')
...
def rename_episodes(path):
    ...
    for item in lfd['spliced_data']:
        ...
        if not rn_path.exists() and not lfd['episode_rename_skip'] :
            choice = input(f"{c.blue}Rename Episode ? : {c.yellow}{original_name} -> {c.green}EP {item} - {recomended_name} {ext}{c.o} [y/n]")
            if choice == 'y' or choice == 'Y':
    ...
    for node in Path(main_path).rglob('*'):
        ...
        if node.is_dir():
            if Path.joinpath(node,'Ξ.lock').exists() :
                ...
                l_file = open(Path.joinpath(node,'Ξ.lock'), 'r',encoding='utf-8')
                data = l_file.read()
                l_file.close()
                data = eval(data)
                ...
                if data['has_folder_data'] == False:
                    ...
                    print(f'{c.green}[Lock file] folder data already fetched. {c.o}\n')
                
                # must have folder data to be predictable 
                if data['has_prediction'] == False:
                    _generate_predictions(node)
                else:
                    print(f'{c.green}[Lock file] predictions already fetched. {c.o}\n')
                ...
                if data['has_episode_data'] == False:
                    ...
                    print(f'{c.green}[Lock file] episode data already fetched. {c.o}\n')
                
                if data['has_anime_data'] == False:
                    _generate_anime_data(node)
                else:
                    print(f'{c.green}[Lock file] anime data already fetched. {c.o}\n')
                
                if data['has_file_data'] == False:
                    _generate_file_data(node)
                else:
                    print(f'{c.green}[Lock file]  file data already fetched. {c.o}\n')
